...
def obtener_pelicula_por_id(id_pelicula):
    ...
    conexion.disconnect()
    objeto_pelicula = clases.Pelicula(id = pelicula_obtenida[0], titulo = pelicula_obtenida[1], anio = pelicula_obtenida[2], duracion = pelicula_obtenida[3], pais = pelicula_obtenida[4], reparto = pelicula_obtenida[5], genero = pelicula_obtenida[6], sinopsis = pelicula_obtenida[7])
...
def guardar_cambios_pelicula(pelicula):
    ...
    conexion = conectar()
    cursor = conexion.cursor()
    valores_a_guardar = (pelicula.titulo, pelicula.anio, pelicula.duracion, pelicula.pais, pelicula.reparto, pelicula.genero, pelicula.sinopsis, pelicula.id)
